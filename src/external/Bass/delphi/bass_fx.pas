{=============================================================================
  ...
  NOTE: This unit will work only with BASS_FX version 2.4.9
...
type
  BPMPROCESSPROC = procedure(chan: DWORD; percent: FLOAT; user: Pointer); {$IFDEF WIN32}stdcall{$ELSE}cdecl{$ENDIF};
  BPMPROC = procedure(chan: DWORD; bpm: FLOAT; user: Pointer); {$IFDEF WIN32}stdcall{$ELSE}cdecl{$ENDIF};

function BASS_FX_BPM_DecodeGet(chan: DWORD; startSec, endSec: DOUBLE; minMaxBPM, flags: DWORD; proc: BPMPROCESSPROC; user: Pointer): FLOAT; {$IFDEF WIN32}stdcall{$ELSE}cdecl{$ENDIF}; external bass_fxdll;
